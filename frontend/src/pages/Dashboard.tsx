...
const Dashboard = () => {
  ...
  const stats = [
    { label: 'Total Balance', value: `$${parseFloat(data?.totalBalance || '0').toLocaleString()}`, icon: <Wallet size={18} />, color: 'var(--primary)', bg: 'rgba(79, 70, 229, 0.05)' },
    { label: 'Total Income', value: `$${parseFloat(data?.totalIncome || '0').toLocaleString()}`, icon: <ArrowUpCircle size={18} />, color: 'var(--accent)', bg: 'rgba(16, 185, 129, 0.05)' },
    { label: 'Total Expense', value: `$${parseFloat(data?.totalExpense || '0').toLocaleString()}`, icon: <ArrowDownCircle size={18} />, color: 'var(--danger)', bg: 'rgba(239, 68, 68, 0.05)' },
  ];

  const chartData = [
    { name: 'W1', income: 4000, expense: 2400 },
    { name: 'W2', income: 3000, expense: 1398 },
    { name: 'W3', income: 2000, expense: 9800 },
    { name: 'W4', income: 2780, expense: 3908 },
  ];
              ...
              {data?.recentTransactions.length === 0 ? (
                ...
                data?.recentTransactions.map((t, i) => (
                  <div key={t.id} style={{ 
                    ...
                    padding: '12px 0', borderBottom: i === data.recentTransactions.length - 1 ? 'none' : '1px solid var(--border)' 
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{ fontSize: '0.875rem', fontWeight: 600 }}>{t.description}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{new Date(t.date).toLocaleDateString()}</div>
                    </div>
                    <div style={{ fontWeight: 700, fontSize: '0.875rem', color: t.type === 'income' ? 'var(--accent)' : 'var(--text-primary)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div style={{ padding: '24px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '8px' }}>
              ${parseFloat(data?.monthlySpent || '0').toLocaleString()} <span style={{ fontSize: '0.875rem', fontWeight: 400, color: 'var(--text-secondary)' }}>/ ${parseFloat(data?.monthlyExpenseLimit || '0').toLocaleString()}</span>
            </div>
            <div style={{ height: '4px', background: 'var(--border)', borderRadius: '2px', overflow: 'hidden', marginBottom: '16px' }}>
              <div style={{ 
                height: '100%', 
                width: `${Math.min(100, (parseFloat(data?.monthlySpent || '0') / parseFloat(data?.monthlyExpenseLimit || '1')) * 100)}%`, 
